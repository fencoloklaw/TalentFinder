...
describe('promise', function () {
    ...
    describe('thenFinally', function () {
        it('nonFailingCallbackDoesNotSuppressOriginalError', function () {
            ...
            return promise.rejected(new StubError).thenFinally(function () {
            }).catch(done).thenFinally(done.assertCalled);
        });

        it('failingCallbackSuppressesOriginalError', function () {
            var done = callbackHelper(assertIsStubError);
            return promise.rejected(new Error('original')).thenFinally(throwStubError).catch(done).thenFinally(done.assertCalled);
        });

        it('callbackThrowsAfterFulfilledPromise', function () {
            var done = callbackHelper(assertIsStubError);
            return promise.fulfilled().thenFinally(throwStubError).catch(done).thenFinally(done.assertCalled);
        });

        it('callbackReturnsRejectedPromise', function () {
            var done = callbackHelper(assertIsStubError);
            return promise.fulfilled().thenFinally(function () {
    ...
    describe('testLongStackTraces', function () {
        ...
        it('doesNotAppendStackIfFeatureDisabled', function () {
            ...
            return promise.rejected(error).then(fail).then(fail).then(fail).then(fail, function (e) {
