    ...
    var _loop_1 = function (fileToLint) {
        ...
        try {
            ...
            if (stat.isFile()) {
                ...
                var fixes = failures.filter(function (f) {
                    return f.hasFix();
                }).map(function (f) {
                    return f.getFix();
                });
...
function consoleTestResultHandler(testResult) {
    ...
    for (var _i = 0, _a = Object.keys(testResult.results); _i < _a.length; _i++) {
        ...
        var didMarkupTestPass = !markupDiffResults.some(function (diff) {
            return !!diff.added || !!diff.removed;
        });
        var didFixesTestPass = !fixesDiffResults.some(function (diff) {
            return !!diff.added || !!diff.removed;
        });
