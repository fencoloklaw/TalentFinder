...
function callExpression(dec) {
    return function_1.Maybe.lift(dec.expression)
        .fmap(function (expr) {
            return isCallExpression(expr) ? expr : undefined;
        });
}
...
function objectLiteralExpression(expr) {
    return function_1.Maybe.lift(expr.arguments[0])
        .fmap(function (arg0) {
            return (isObjectLiteralExpression(arg0) ? arg0 : undefined);
        });
}
...
function withIdentifier(identifier) {
    return function_1.ifTrue(function (expr) {
        return (isIdentifier(expr.expression) && expr.expression.text === identifier);
    });
}
...
function getStringInitializerFromProperty(propertyName, ps) {
    var property = ps.find(function (p) {
        return isProperty(propertyName, p);
    });
    return getInitializer(property)
        .fmap(function (expr) {
            return isSimpleTemplateString(expr) ? expr : undefined;
        });
}
