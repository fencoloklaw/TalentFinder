...
functions.findBindings = function (binding, exactMatch, using, rootSelector) {
    ...
    if (angular.getTestability) {
        return angular.getTestability(root).findBindings(using, binding, exactMatch);
    }
...
functions.findByModel = function (model, using, rootSelector) {
    ...
    if (angular.getTestability) {
        return angular.getTestability(root).findModels(using, model, true);
    }
    var prefixes = ['ng-', 'ng_', 'data-ng-', 'x-ng-', 'ng\\:'];
    for (var p = 0; p < prefixes.length; ++p) {
...
functions.setLocation = function (selector, url) {
    var el = document.querySelector(selector);
    if (angular.getTestability) {
        return angular.getTestability(el).setLocation(url);
    }
