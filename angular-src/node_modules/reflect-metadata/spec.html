...
<emu-intro id="introduction">
    ...
    <emu-note>A shim for this API can be found here: <a href="https://github.com/rbuckton/ReflectDecorators">https://github.com/rbuckton/ReflectDecorators</a>
    </emu-note>
...
<emu-clause id="abstract-operations">
    ...
    <emu-clause id="operations-on-objects">
        ...
        <emu-clause id="getorcreatemetadatamap" aoid="GetOrCreateMetadataMap">
            ...
            <p>When the abstract operation GetOrCreateMetadataMap is called with Object _O_, property key _P_, and
                Boolean _Create_ the following steps are taken:</p>
            <emu-alg>
...
<emu-clause id="ordinary-and-exotic-objects-behaviors">
    ...
    <emu-clause id="ordinary-object-internal-methods-and-internal-slots">
        ...
        <p>All ordinary objects have an internal slot called [[Metadata]]. The value of this internal slot is either
            *null* or a *Map* object and is used for storing metadata for an object.</p>
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-hasmetadata">
            <h1>[[HasMetadata]] ( MetadataKey, P )</h1>
            <p>When the [[HasMetadata]] internal method of _O_ is called with ECMAScript language value _MetadataKey_
                and property key _P_, the following steps are taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinaryhasmetadata" aoid="OrdinaryHasMetadata">
                ...
                <p>When the abstract operation OrdinaryHasMetadata is called with ECMAScript language value
                    _MetadataKey_, Object _O_, and property key _P_, the following steps are taken:</p>
                <emu-alg>
                    1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-hasownmetadata">
            ...
            <p>When the [[HasOwnMetadata]] internal method of _O_ is called with ECMAScript language value _MetadataKey_
                and property key _P_, the following steps are taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinaryhasownmetadata" aoid="OrdinaryHasOwnMetadata">
                ...
                <p>When the abstract operation OrdinaryHasOwnMetadata is called with ECMAScript language value
                    _MetadataKey_, Object _O_, and property key _P_, the following steps are taken:</p>
                <emu-alg>
                    1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-getmetadata">
            ...
            <p>When the [[GetMatadata]] internal method of _O_ is called with ECMAScript language value _MetadataKey_
                and property key _P_, the following steps are taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinarygetmetadata" aoid="OrdinaryGetMetadata">
                ...
                <p>When the abstract operation OrdinaryGetMetadata is called with ECMAScript language value
                    _MetadataKey_, Object _O_, and property key _P_, the following steps are taken:</p>
                <emu-alg>
                    1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
                    2. Let _hasOwn_ be ? OrdinaryHasOwnMetadata(_MetadataKey_, _O_, _P_).
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-getownmetadata">
            ...
            <p>When the [[GetOwnMetadata]] internal method of _O_ is called with ECMAScript language value _MetadataKey_
                and property key _P_, the following steps are taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinarygetownmetadata" aoid="OrdinaryGetOwnMetadata">
                ...
                <p>When the abstract operation OrdinaryGetOwnMetadata is called with ECMAScript language value
                    _MetadataKey_, Object _O_, and property key _P_, the following steps are taken:</p>
                <emu-alg>
                    1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
                    2. Let _metadataMap_ be ? GetOrCreateMetadataMap(_O_, _P_, *false*).
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-defineownmetadata">
            ...
            <p>When the [[DefineOwnMetadata]] internal method of _O_ is called with ECMAScript language value
                _MetadataKey_, ECMAScript language value _MetadataValue_, and property key _P_, the following steps are
                taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinarydefineownmetadata" aoid="OrdinaryDefineOwnMetadata">
                ...
                <p>When the abstract operation OrdinaryDefineOwnProperty is called with ECMAScript language value
                    _MetadataKey_, ECMAScript language value _MetadataValue_, Object _O_, and property key _P_, the
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-metadatakeys">
            ...
            <p>When the [[MetadataKeys]] internal method of _O_ is called with property key _P_ the following steps are
                taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinarymetadatakeys" aoid="OrdinaryMetadataKeys">
                ...
                <p>When the abstract operation OrdinaryMetadataKeys is called with Object _O_ and property key _P_ the
                    following steps are taken:</p>
                <emu-alg>
                    1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-ownmetadatakeys">
            ...
            <p>When the [[OwnMetadataKeys]] internal method of _O_ is called with property key _P_ the following steps
                are taken:</p>
            <emu-alg>
            ...
            <emu-clause id="ordinaryownmetadatakeys" aoid="OrdinaryOwnMetadataKeys">
                ...
                <p>When the abstract operation OrdinaryOwnMetadataKeys is called with Object _O_ and property key _P_
        ...
        <emu-clause id="ordinary-object-internal-methods-and-internal-slots-deletemetadata">
            ...
            <p>When the [[DeleteMetadata]] internal method of _O_ is called with ECMAScript language value _MetadataKey_
                and property key _P_ the following steps are taken:</p>
            <emu-alg>
                1. Assert: _P_ is *undefined* or IsPropertyKey(_P_) is *true*.
                2. Let _metadataMap_ be ? GetOrCreateMetadataMap(_O_, _P_, *false*).
...
<emu-clause id="reflection">
    ...
    <emu-clause id="reflect">
        ...
        <emu-clause id="reflect-metadatadecoratorfunctions">
            ...
            <p>When a metadata decorator function _F_ is called with arguments _target_ and _key_, the following steps
                are taken:</p>
            <emu-alg>
                1. Assert: _F_ has a [[MetadataKey]] internal slot whose value is an ECMAScript language value, or
                *undefined*.
                2. Assert: _F_ has a [[MetadataValue]] internal slot whose value is an ECMAScript language value, or
                *undefined*.
        ...
        <emu-clause id="reflect.metadata">
            ...
            <p>When the `metadata` function is called with arguments _metadataKey_ and _metadataValue_, the following
                steps are taken:</p>
            <emu-alg>
        ...
        <emu-clause id="reflect.definemetadata">
            ...
            <p>When the `defineMetadata` function is called with arguments _metadataKey_, _metadataValue_, _target_, and
                _propertyKey_, the following steps are taken:</p>
            <emu-alg>
        ...
        <emu-clause id="reflect.hasmetadata">
            ...
            <p>When the `hasMetadata` function is called with arguments _metadataKey_, _target_, and _propertyKey_, the
                following steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.hasownmetadata">
            ...
            <p>When the `hasOwnMetadata` function is called with arguments _metadataKey_, _target_, and _propertyKey_,
                the following steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.getmetadata">
            ...
            <p>When the `getMetadata` function is called with arguments _metadataKey_, _target_, and _propertyKey_, the
                following steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.getownmetadata">
            ...
            <p>When the `getOwnMetadata` function is called with arguments _metadataKey_, _target_, and _propertyKey_,
                the following steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.getmetadatakeys">
            ...
            <p>When the `getMetadataKeys` function is called with arguments _target_ and _propertyKey_, the following
                steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.getownmetadatakeys">
            ...
            <p>When the `getOwnMetadataKeys` function is called with arguments _target_ and _propertyKey_, the following
                steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
        ...
        <emu-clause id="reflect.deletemetadata">
            ...
            <p>When the `deleteMetadata` function is called with arguments _metadataKey_, _target_, and _propertyKey_,
                the following steps are taken:</p>
            <emu-alg>
                1. If Type(_target_) is not Object, throw a *TypeError* exception.
