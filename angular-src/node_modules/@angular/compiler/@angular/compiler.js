...
class Xliff extends Serializer {
    ...
    write(messages, locale) {
        ...
        const /** @type {?} */ xliff = new Tag('xliff', {
            version: _VERSION,
            xmlns: _XMLNS
...
class Xliff2 extends Serializer {
    ...
    write(messages, locale) {
        ...
        const /** @type {?} */ xliff = new Tag(_XLIFF_TAG, {
            version: _VERSION$1,
            xmlns: _XMLNS$1,
...
class ProviderElementContext {
     ...
     * @return {?}
     */
    get transformedHasViewContainer() {
...
class DirectiveNormalizer {
    ...
    clearCacheFor(normalizedDirective) {
        ...
        template.externalStylesheets.forEach((stylesheet) => {
            this._resourceLoaderCache.delete(/** @type {?} */ ((stylesheet.moduleUrl)));
        });
    }
    ...
    normalizeLoadedTemplate(prenormData, template, templateAbsUrl) {
        ...
        const /** @type {?} */ rootNodesAndErrors = this._htmlParser.parse(template, templateSourceUrl({reference: prenormData.ngModuleType}, {type: {reference: prenormData.componentType}}, {
            isInline,
        ...
        return Promise
            .all(styleUrls.filter((styleUrl) => !loadedStylesheets.has(styleUrl))
                .map(styleUrl => this._fetch(styleUrl).then((loadedStyle) => {
                    const /** @type {?} */ stylesheet = this.normalizeStylesheet(new CompileStylesheetMetadata({
                        styles: [loadedStyle],
                        moduleUrl: styleUrl
                    }));
...
class DirectiveResolver {
    ...
    _extractPublicName(def) {
        return splitAtColon(def, [/** @type {?} */ ((null)), def])[1].trim();
    }
...
function debugOutputAstAsTypeScript(ast) {
    const /** @type {?} */ converter = new _TsEmitterVisitor(_debugFilePath, {
         ...
         * @return {?}
         */
        fileNameToModuleName(filePath, containingFilePath) {
            return filePath;
        },
        /**
         * @param {?} symbol
...
class ViewBuilder {
    ...
    visitBoundText(ast, context) {
        ...
        const /** @type {?} */ updateRendererExpressions = inter.expressions.map((expr, bindingIndex) => this._preprocessUpdateExpression({
            nodeIndex,
            bindingIndex,
            sourceSpan: ast.sourceSpan,
            context: COMP_VAR,
...
function serializeSummaries(summaryResolver, symbolResolver, symbols, types) {
    ...
    // CompileTypeSummary.
    symbols.forEach((resolvedSymbol) => serializer.addOrMergeSummary({
...
class JitCompiler {
    ...
    _compileTemplate(template) {
        ...
        stylesCompileResult.externalStylesheets.forEach((r) => {
            externalStylesheetsByModuleUrl.set(/** @type {?} */ ((r.meta.moduleUrl)), r);
        });
