...
class Routes {
    ...
    routes(app) {
        ...
        app.route('/updateProfile')
            .put('/updateProfile', passport.authenticate('jwt', { session: false }), (req, res) => {
            userModel_1.UserSchema.updateUser(req, (err, data) => {
                if (err)
                    throw err;
                res.json({ user: data });
            });
        ...
        app.route('/search')
            .post((req, res, next) => {
            userModel_1.UserSchema.getMatchingUsers(req, (err, data) => {
                if (err)
                    throw err;
                if (!data) {
                    return res.json({ success: false, msg: 'User not found' });
